...
export const defaultSettings: Record<string, string> = {
  ...
  about_title: "作品不是终点，\n而是我理解世界的索引。",
  about_text: "作品不是终点，而是我理解世界的索引。你好，我是 RING，一名横跨视觉、空间与影像的独立创作者。",
...
export function errorMessage(error: unknown) {
  ...
  try { return JSON.stringify(error); } catch { return String(error || "未知错误"); }
}

export function isSupabaseConfigError(error: unknown) {
  return /Missing NEXT_PUBLIC_SUPABASE|Invalid API key|fetch failed|relation .* does not exist|schema cache|Could not find|\[object Object\]|JWT|permission denied|row-level security|PGRST/i.test(errorMessage(error));
}
