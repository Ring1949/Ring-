...
export function PromptLibraryClient(){
  ...
  const detailIndex=detail?visible.findIndex((item)=>item.id===detail.id):-1;
  ...
  const remove=async(item:Prompt)=>{setMenu("");if(!confirm(`确定删除提示词「${item.title}」${item.image_url?"及其预览图片":""}吗？`))return false;const response=await fetch(`/api/prompts/${item.id}`,{method:"DELETE"});const payload=await response.json();if(!response.ok){alert(payload.error||"删除失败");return false}setPrompts((all)=>all.filter((entry)=>entry.id!==item.id));return true};
  ...
  return <main className={styles.page} onClick={()=>setMenu("")}>
    ...
    {detail?<PromptDetail prompt={detail} copied={copied===detail.id} close={()=>setDetail(null)} copy={()=>void copyPrompt(detail)} previous={()=>moveDetail(-1)} next={()=>moveDetail(1)} canNavigate={visible.length>1}/>:null}
...
function PromptDetail({prompt,copied,close,copy,previous,next,canNavigate}:{prompt:Prompt;copied:boolean;close:()=>void;copy:()=>void;previous:()=>void;next:()=>void;canNavigate:boolean}){
  ...
  return <div className={styles.backdrop} onMouseDown={(event)=>{if(event.currentTarget===event.target)close()}}>{canNavigate?<div className={styles.detailNav}><button type="button" aria-label="上一个提示词" onClick={previous}>←</button><button type="button" aria-label="下一个提示词" onClick={next}>→</button></div>:null}<article className={styles.detail} role="dialog" aria-modal="true" aria-label={`${prompt.title} 详情`}><button className={styles.detailClose} onClick={close} aria-label="关闭">×</button>{prompt.image_url?<img src={prompt.image_url} alt={`${prompt.title} 效果预览`}/>:null}<small>{prompt.category}</small><h2>{prompt.title}</h2><p>{prompt.description}</p><pre>{prompt.content}</pre><footer><div className={styles.tags}>{prompt.tags.slice(0,2).map((tag)=><span key={tag}>{tag}</span>)}</div><div className={styles.detailActions}><button type="button" onClick={copy}>{copied?"已复制":"复制完整 Prompt"}</button></div></footer></article></div>
}
