...
export function SkillLibraryClient() {
  ...
  const load = async () => { const response = await fetch("/api/skills", { cache: "no-store" }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || "Skill 库读取失败"); setSkills(payload.skills || []); setCategories(payload.categories || []); setStatus(""); };
