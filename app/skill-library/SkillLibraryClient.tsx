...
export function SkillLibraryClient() {
  ...
  const visible = useMemo(() => { const needle = query.trim().toLocaleLowerCase("zh-CN"); return skills.filter((skill) => !needle || [skill.name, skill.invocation, skill.description, skill.category_name, skill.original_name].join(" ").toLocaleLowerCase("zh-CN").includes(needle)); }, [query, skills]);
  ...
  const copyInvocation = async (skill: Skill) => { try { await copyText(skill.invocation?.trim() || `@${skill.name}`); setCopied(skill.id); setTimeout(() => setCopied((current) => current === skill.id ? "" : current), 1500); } catch (error) { alert(error instanceof Error ? error.message : "复制失败"); } };
    ...
    <section className={styles.library}>
      ...
      <section className={styles.grid} aria-live="polite">{pageItems.map((skill) => { const hasPreview = Boolean(skill.preview_url); return <article key={skill.id} className={`${styles.card} ${hasPreview ? styles.cardWithImage : styles.cardPlain}`} style={{ ["--card-tone" as string]: toneFor(skill.id) }}>
        ...
        <div className={styles.cardBody}><div className={styles.cardActions}>
          ...
          <button type="button" className={styles.copyButton} aria-label={copied === skill.id ? `${skill.name} 调用内容已复制` : `复制 ${skill.invocation || `@${skill.name}`}`} title={copied === skill.id ? "已复制" : `复制 ${skill.invocation || `@${skill.name}`}`} onClick={(event) => { event.stopPropagation(); void copyInvocation(skill); }}><CopyIcon checked={copied === skill.id}/>{copied === skill.id ? <span className={styles.copyTip}>已复制调用内容</span> : null}</button>
...
function SkillEditor({ skill, categories, busy, setBusy, close, saved }: { skill: Skill | null; categories: Category[]; busy: boolean; setBusy: (value: boolean) => void; close: () => void; saved: () => Promise<void> }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const body: Record<string, unknown> = { name: values.get("name"), invocation: values.get("invocation"), category_name: values.get("category_name"), description: values.get("description") };
  ...
  return <div className={styles.backdrop} onMouseDown={(event) => { if (event.currentTarget === event.target) close(); }}><section className={styles.editor} role="dialog" aria-modal="true"><div className={styles.editorHead}><div><p>{skill ? "EDIT SKILL" : "NEW SKILL"}</p><h2>{skill ? "编辑 Skill" : "添加 Skill"}</h2></div><button type="button" onClick={close} aria-label="关闭">×</button></div><form onSubmit={submit}><label><span>Skill 展示名称</span><input name="name" defaultValue={skill?.name} required maxLength={80}/></label><label><span>分类（可直接新建）</span><input name="category_name" defaultValue={skill?.category_name} list="skill-categories" required maxLength={40}/><datalist id="skill-categories">{categories.map((category) => <option key={category.id} value={category.name}/>)}</datalist></label><label className={styles.wide}><span>复制调用内容</span><input name="invocation" defaultValue={skill?.invocation || (skill ? `@${skill.name}` : "")} required maxLength={240} placeholder="例如：@visual-design 或完整调用指令"/></label><label className={styles.wide}><span>说明</span><textarea name="description" defaultValue={skill?.description} rows={4} maxLength={400} placeholder="说明这个 Skill 解决什么问题、如何使用。"/></label>
