...
export function AdminUploadsClient() {
  ...
  const load = async () => {
    const [meResponse, categoryResponse, projectResponse, mediaResponse] = await Promise.all([
      fetch("/api/me", { cache: "no-store" }), fetch("/api/categories", { cache: "no-store" }),
      fetch("/api/projects", { cache: "no-store" }), fetch("/api/blob-media", { cache: "no-store" })
    ]);
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (tooLarge) throw new Error(`${tooLarge.name} 超过 250 MB。删除照片不能改变单文件上限；请压缩或分卷上传。`);
  ...
  return <>
    ...
    <div className={styles.content}>
      <section className={styles.intro}><div><p>DURABLE MEDIA UPLOAD</p><h1>上传作品</h1></div><div className={styles.health}><b>{health?.ok ? "现有持久存储正常" : health?.error ? "现有存储暂不可写" : "正在检查现有存储"}</b>{health?.storage ? `${health.storage.object_count} 个文件 · ${health.storage.size_mb} MB` : health?.error || "不会写入 Vercel 临时目录"}</div></section>
      <section className={styles.panel}><h2>选择文件与作品信息</h2><form className={styles.form} onSubmit={submit}>
        <label className={`${styles.filePicker} ${styles.fieldWide}`}><input name="files" type="file" accept="image/*,video/*,.pdf,.zip,.doc,.docx,.txt" multiple required={!pending.length} /><span>{pending.length ? `${pending.length} 个文件已上传，可重试保存资料` : "点击选择图片、视频或文档；大文件将自动分片直传"}</span></label>
        ...
        <div className={styles.field}><label>归属系列（可空）</label><select name="project_id"><option value="">不归属具体系列</option>{projects.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></div>
      ...
      </form><p className={styles.note}>所有文件直接进入 Vercel Blob 持久对象存储，上传后再校验大小与路径；站点部署、重启或闲置不会清空文件。</p></section>
