...
export async function POST(request:NextRequest){
  ...
  try{const body=await request.json(),title=text(body.title,80),author=text(body.author,80);if(!title||!author)return NextResponse.json({error:"标题和作者不能为空。"},{status:400});if(!body.image)return NextResponse.json({error:"请选择封面图片。"},{status:400});await verifyPoetryImage(body.image);const now=new Date().toISOString();const card:PoetryCardRecord={id:crypto.randomUUID(),title,author,image_url:text(body.image.url,2000),image_pathname:text(body.image.pathname,1000),image_name:text(body.image.name,240),storage_provider:"r2",object_key:text(body.image.object_key,1000),created_at:now,updated_at:now};const manifest=await getPoetryManifest();await savePoetryManifest({...manifest,cards:[...manifest.cards,card]});return NextResponse.json({card},{status:201})}catch(error){return NextResponse.json({error:error instanceof Error?error.message:"诗词卡片保存失败"},{status:400})}
}
