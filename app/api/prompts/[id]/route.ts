...
export async function PUT(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const image = body.image === undefined ? { url: previous.image_url, pathname: previous.image_pathname, name: previous.image_name }
      : body.image || { url: "", pathname: "", name: "" };
    const updated = {
    ...
    if (previous.image_url && previous.image_url !== updated.image_url) await removePromptImage(previous.image_url).catch(() => undefined);
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (previous.image_url) await removePromptImage(previous.image_url).catch(() => undefined);
