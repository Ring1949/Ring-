...
type SavedCard={id:string;title:string;author:string;position_x?:number;position_y?:number;image_url:string;image_pathname:string;image_name:string};
type CardView={id:string;title:string;author:string;src:string;position:string};
const poems:Poem[]=[
...
export function PoetryLibraryClient(){
  ...
  const cards=useMemo<CardView[]>(()=>[...poems.map((poem,index)=>({id:poem.id,title:poem.title,author:`${poem.dynasty} · ${poem.author}`,src:poem.image||cover(poem,index),position:"50% 50%"})),...saved.map((card)=>({id:card.id,title:card.title,author:card.author,src:card.image_url,position:`${card.position_x??50}% ${card.position_y??50}%`}))],[saved]);
  ...
  return <main className={styles.page} style={{position:"fixed",inset:0}}>
    ...
    {selected?<div className={overlay.lightbox} role="dialog" aria-modal="true" aria-label={`${selected.title}完整图片`} onClick={()=>setSelected(null)}><button aria-label="关闭完整图片">×</button><img src={selected.src} alt={`${selected.title}完整图片`} onClick={(event)=>event.stopPropagation()}/></div>:null}
