...
type SavedCard={id:string;title:string;author:string;image_url:string;image_pathname:string;image_name:string};
type CardView={id:string;title:string;author:string;src:string};
const poems:Poem[]=[
...
export function PoetryLibraryClient(){
  ...
  const cards=useMemo<CardView[]>(()=>[...poems.map((poem,index)=>({id:poem.id,title:poem.title,author:`${poem.dynasty} · ${poem.author}`,src:poem.image||cover(poem,index)})),...saved.map((card)=>({id:card.id,title:card.title,author:card.author,src:card.image_url}))],[saved]);
  const slides=useMemo<CoverflowSlide[]>(()=>cards.map((card)=>({src:card.src,alt:`${card.title}封面`,title:card.title,subtitle:card.author})),[cards]);
  return <main className={styles.page} style={{position:"fixed",inset:0}}>
    ...
    {selected?<div className={overlay.lightbox} role="dialog" aria-modal="true" onClick={()=>setSelected(null)}><button aria-label="关闭全图">×</button><img src={selected.src} alt={`${selected.title}完整封面`}/><div><b>{selected.title}</b><span>{selected.author}</span></div></div>:null}
