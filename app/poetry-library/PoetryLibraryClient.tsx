...
export function PoetryLibraryClient(){
  const [query,setQuery]=useState(""),[selected,setSelected]=useState<Poem|null>(null),[uploads,setUploads]=useState<CoverflowSlide[]>([]),[notice,setNotice]=useState("");const fileRef=useRef<HTMLInputElement>(null);
  const filtered=useMemo(()=>{const needle=query.trim().toLocaleLowerCase("zh-CN");return poems.filter((poem)=>!needle||`${poem.title} ${poem.author} ${poem.dynasty} ${poem.theme} ${poem.lines.join(" ")}`.toLocaleLowerCase("zh-CN").includes(needle))},[query]);
  const slides=useMemo<CoverflowSlide[]>(()=>[...filtered.map((poem)=>({src:poem.image||cover(poem,poems.indexOf(poem)),alt:`${poem.title}诗词封面`,title:poem.title,subtitle:`${poem.dynasty} · ${poem.author}`,meta:[{label:"主题",value:poem.theme},{label:"作者",value:`${poem.dynasty} · ${poem.author}`}]})),...uploads],[filtered,uploads]);
  const addPreview=(file:File|null)=>{if(!file)return;if(!file.type.startsWith("image/")){setNotice("请选择图片文件。");return}const url=URL.createObjectURL(file);setUploads((current)=>[...current,{src:url,alt:file.name,title:"新诗词卡片",subtitle:"图片已载入，等待补充诗词资料",meta:[{label:"图片",value:file.name}]}]);setNotice("图片卡片已加入当前预览。你把正式图片交给我后，我会写入网站并永久保存。");};
  return <main className={styles.page}>
    <header className={styles.header}><Link href="/" className={styles.brand}>Ring</Link><nav><Link href="/#extensions">扩展</Link><Link href="/prompt-library">Prompt 库</Link></nav></header>
    <section className={styles.hero}><div><p>POETRY APPRECIATION</p><h1>诗词鉴赏</h1><span>以诗会友，以词传情。在古人的句子里，遇见更广阔的世界。</span></div><button className={styles.add} aria-label="添加诗词卡片" title="添加诗词卡片" onClick={()=>fileRef.current?.click()}>＋</button><input ref={fileRef} type="file" accept="image/*" hidden onChange={(event)=>addPreview(event.target.files?.[0]||null)}/></section>
    <section className={styles.search}><label><span>⌕</span><input value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="搜索诗名、作者、名句或主题"/></label>{notice?<p>{notice}</p>:null}</section>
    <section className={styles.carousel}>{slides.length?<CoverflowCarousel slides={slides} cardWidth="clamp(210px, 28vw, 360px)" onSelect={(index)=>{if(index<filtered.length)setSelected(filtered[index])}}/>:<div className={styles.empty}>没有找到符合条件的诗词。</div>}<p className={styles.hint}>左右拖动卡片 · 点击中央卡片阅读鉴赏 · 键盘方向键也可切换</p></section>
